...
function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (result.error) {
        toast.error(result.error.message ?? "No se pudo iniciar Google");
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-semibold">{mode === "signin" ? "Inicia sesión" : "Crea tu cuenta"}</h2>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-background px-2 text-muted-foreground">o con email</span></div>
          </div>
